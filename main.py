...
#get minority_upsampling_train, validatio, minority_upsampling_test data split
...
def test(epoch = 0):
    ...
    utils.print_class_acc(output[idx_test], labels[idx_test], class_num_mat[:,2], pre='minority_upsampling_test')
...
# 将 minority_upsampling_train accuracy 保存到 "tensorboard/minority_upsampling_train" 文件夹
log_dir = os.path.join('tensorboard', 'All_reweight_train')
train_writer = SummaryWriter(log_dir=log_dir)
# 将 minority_upsampling_test accuracy 保存到 "tensorboard/minority_upsampling_test" 文件夹
log_dir = os.path.join('tensorboard', 'All_reweight_test')
test_writer = SummaryWriter(log_dir=log_dir)

for epoch in range(args.epochs):
    train_acc = train(epoch)
    test_acc = test()
    train_writer.add_scalar('Train Accuracy', train_acc, epoch)
    test_writer.add_scalar('Test Accuracy', test_acc, epoch)
    if epoch % 10 == 0:
        test(epoch)
